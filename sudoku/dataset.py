"""Datasets and data-loading utilities for the Sudoku solving task.

Classes
-------
Sudoku_Dataset
    Dataset backed by pre-computed pickle files (SATNet / 70k splits).
Sudoku_Dataset_Palm
    Dataset using the PALM hard-Sudoku benchmark (auto-downloaded).

Helpers
-------
save_pickle / load_pickle
    Thin wrappers around ``pickle.dump`` / ``pickle.load``.
"""
...
def save_pickle(obj: object, filename: str) -> None:
    """Serialise *obj* to *filename* using the highest available pickle protocol."""
    with open(filename, "wb") as f:
        pickle.dump(obj, f, protocol=pickle.HIGHEST_PROTOCOL)


def load_pickle(filename: str) -> object:
    """Deserialise an object from *filename*."""
...
class Sudoku_Dataset_Palm(Dataset):
    def __init__(self, segment, limit, seed):
        ...
        if not os.path.exists(dest_dir):
            ...
            logger.info(f"Downloading Palm dataset into {dest_dir}...")
