...
class MultiInceptionMetrics(Metric):
    ...
    def update(self, images, labels=None, image_type="unconditional") -> None:  # type: ignore
        ...
        if image_type == "conditional" and labels is None:
            raise ValueError("Argument `labels` expected to be provided when `image_type` is 'conditional'.")
        if image_type == "unconditional" and labels is not None:
            raise ValueError("Argument `labels` expected to be None when `image_type` is 'unconditional'.")
        if image_type == "unconditional" and not self.compute_unconditional_metrics:
            raise ValueError("Argument `image_type` is 'unconditional', but `compute_unconditional_metrics` is False.")
        if image_type == "conditional" and not self.compute_conditional_metrics:
            raise ValueError("Argument `image_type` is 'conditional', but `compute_conditional_metrics` is False.")
        ...
        if image_type == "real":
            ...
            if self.compute_conditional_metrics_per_class:
                for i in range(self.num_classes):
                    getattr(self, f"real_cond_features_{i}").append(features[labels.argmax(dim=-1) == i])
            ...
            if self.compute_conditional_metrics_per_class:
                for i in range(self.num_classes):
                    getattr(self, f"fake_cond_features_{i}").append(features[labels.argmax(dim=-1) == i])
                    getattr(self, f"fake_cond_logits_{i}").append(logits[labels.argmax(dim=-1) == i])
    ...
    def compute(self) -> Tensor:
        ...
        if self.compute_unconditional_metrics:
            ...
            output_metrics["fid_unconditional"] = self.fid(real_features, fake_uncond_features)
            output_metrics["inception_score_unconditional"] = self.inception_score(fake_uncond_logits)
            (
            ...
            ) = self.manifold_metrics(real_features, fake_uncond_features, self.manifold_k)
        ...
        if self.compute_conditional_metrics:
            ...
            ) = self.manifold_metrics(real_features, fake_cond_features, self.manifold_k)
        ...
        if self.compute_conditional_metrics_per_class:
            ...
            for i in range(self.num_classes):
                ...
                real_features_per_class = torch.cat(getattr(self, f"real_cond_features_{i}"), dim=0)
                fake_cond_features_per_class = torch.cat(getattr(self, f"fake_cond_features_{i}"), dim=0)
                fake_cond_logits_per_class = torch.cat(getattr(self, f"fake_cond_logits_{i}"), dim=0)
                fid_per_class += self.fid(real_features_per_class, fake_cond_features_per_class)
            ...
            output_metrics["inception_score_conditional_per_class"] = is_per_class / num_classes
            output_metrics["precision_conditional_per_class"] = precision_per_class / num_classes
            output_metrics["recall_conditional_per_class"] = recall_per_class / num_classes
            output_metrics["density_conditional_per_class"] = density_per_class / num_classes
            output_metrics["coverage_conditional_per_class"] = coverage_per_class / num_classes
    ...
    def reset(self) -> None:
        if not self.reset_real_features:
            ...
            if self.compute_conditional_metrics_per_class:
                for i in range(self.num_classes):
                    vars()[f"real_cond_features_{i}"] = deepcopy(getattr(self, f"real_cond_features_{i}"))
            super().reset()
